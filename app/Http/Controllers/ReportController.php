...
{
    // Validation avec tes noms de champs
    $request->validate([
        'date' => 'required|date',
        'time' => 'required|date|after_or_equal:date' // ici 'time' sert de date de fin
    ]);
    ...
    $stats = Order::whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
        ->where('status', 'completed')
        ->select([
    ...
    // Répartition des paiements (Correction des variables ici)
    $paymentMethods = [
        'cash' => Order::whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
            ->where('status', 'completed')->where('payment_method', 'cash')->sum('total'),
        'card' => Order::whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
            ->where('status', 'completed')->where('payment_method', 'card')->sum('total'),
        'mobile_money' => Order::whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
            ->where('status', 'completed')->where('payment_method', 'mobile_money')->sum('total')
    ];
...
// Rapport de vente par catégorie (Filtré par date unique)
    public function categoryReport(Request $request)
    {
        ...
        $categorySales = OrderItem::whereHas('order', function ($query) use ($date) {
                $query->whereDate('created_at', $date)
                      ->where('status', 'completed');
            })
